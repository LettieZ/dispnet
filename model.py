...
class  DispNet(torch.nn.Module):
    ...
    # encoder part start
        self.conv1=torch.nn.Conv2d(in_channels=6,out_channels=64,kernel_size=7,stride=2,padding=3)
        self.conv2=torch.nn.Conv2d(in_channels=64,out_channels=128,kernel_size=5,stride=2,padding=2)
        self.conv3a=torch.nn.Conv2d(in_channels=128,out_channels=256,kernel_size=5,stride=2,padding=2)
        self.conv3b=torch.nn.Conv2d(in_channels=256,out_channels=256,kernel_size=3,stride=1,padding=1)
        self.conv4a=torch.nn.Conv2d(in_channels=256,out_channels=512,kernel_size=3,stride=2,padding=1)
        self.conv4b=torch.nn.Conv2d(in_channels=512,out_channels=512,kernel_size=3,stride=1,padding=1)
        self.conv5a=torch.nn.Conv2d(in_channels=512,out_channels=512,kernel_size=3,stride=2,padding=1)
        self.conv5b=torch.nn.Conv2d(in_channels=512,out_channels=512,kernel_size=3,stride=1,padding=1)
        self.conv6a=torch.nn.Conv2d(in_channels=512,out_channels=1024,kernel_size=3,stride=2,padding=1)
        self.conv6b=torch.nn.Conv2d(in_channels=1024,out_channels=1024,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr6
        self.conv_predict_flow6=torch.nn.Conv2d(in_channels=1024,out_channels=1,kernel_size=3,stride=1,padding=1)
    # encoder part end

    # decoder part start
        self.upconv5=torch.nn.ConvTranspose2d(in_channels=1024,out_channels=512,kernel_size=4,stride=2,padding=1)

        # this layer is to expand pr6 to larger size
        self.upsample_flow6to5=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
        self.iconv5=torch.nn.Conv2d(in_channels=1025,out_channels=512,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr5
        self.conv_predict_flow5=torch.nn.Conv2d(in_channels=512,out_channels=1,kernel_size=3,stride=1,padding=1)
        self.upconv4=torch.nn.ConvTranspose2d(in_channels=512,out_channels=256,kernel_size=4,stride=2,padding=1)

        # this layer is to expand pr5 to larger size
        self.upsample_flow5to4=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
        self.iconv4=torch.nn.Conv2d(in_channels=769,out_channels=256,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr4
        self.conv_predict_flow4=torch.nn.Conv2d(in_channels=256,out_channels=1,kernel_size=3,stride=1,padding=1)
        self.upconv3=torch.nn.ConvTranspose2d(in_channels=256,out_channels=128,kernel_size=4,stride=2,padding=1)

        # this layer is to expand pr4 to larger size
        self.upsample_flow4to3=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
        self.iconv3=torch.nn.Conv2d(in_channels=385,out_channels=128,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr3
        self.conv_predict_flow3=torch.nn.Conv2d(in_channels=128,out_channels=1,kernel_size=3,stride=1,padding=1)
        self.upconv2=torch.nn.ConvTranspose2d(in_channels=128,out_channels=64,kernel_size=4,stride=2,padding=1)

        # this layer is to expand pr3 to larger size
        self.upsample_flow3to2=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
        self.iconv2=torch.nn.Conv2d(in_channels=193,out_channels=64,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr2
        self.conv_predict_flow2=torch.nn.Conv2d(in_channels=64,out_channels=1,kernel_size=3,stride=1,padding=1)
        self.upconv1=torch.nn.ConvTranspose2d(in_channels=64,out_channels=32,kernel_size=4,stride=2,padding=1)

        # this layer is to expand pr2 to larger size
        self.upsample_flow2to1=torch.nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=4,stride=2,padding=1)
        self.iconv1=torch.nn.Conv2d(in_channels=97,out_channels=32,kernel_size=3,stride=1,padding=1)

        # this layer is to produce pr1
        self.conv_predict_flow1=torch.nn.Conv2d(in_channels=32,out_channels=1,kernel_size=3,stride=1,padding=1)
